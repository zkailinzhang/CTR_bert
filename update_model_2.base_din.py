...
flags.DEFINE_string("logfile",'logs.base_din.lr0.8.out',"log file to save")
...
#__file__
FLAGS.csvfile = FLAGS.csvfile+ str(FLAGS.decay_rate) + '_base_din.csv' 
...
class UpdateModel2(UpdateModel):
    ...
    def build_inputs(self):
        ...
        with self.graph.as_default():
            ...
            with tf.name_scope("Embedding_layer"):
               #20*40*128
                self.adimg_embeddings_var2  = tf.get_variable("adimg_embedding_var2", [ADIMG_LABEL_DIM,ADIMG_VALUE_DIM,EMBEDDING_DIM])
                #索引 idx  
                self.adimg_embedded2 = tf.nn.embedding_lookup(self.adimg_embeddings_var2, self.ad_label_ph)
                

                #n*7 ->  n*7*8  做one-hot  -1  00000   1   10000
                self.ad_value_ph_ohot = tf.one_hot(self.ad_value_ph,depth=ADIMG_VALUE_DIM,axis=-1)
                
                    #n*7*8  n*7*1 *8 
                self.ad_value_ph_ohot = tf.expand_dims(self.ad_value_ph_ohot,axis=-2)
                #n*7*8*128  就是对应相乘，
                self.adimg_embedded = tf.matmul(self.ad_value_ph_ohot ,self.adimg_embedded2)
...
def prepare_data(feature: List, target: List, choose_len: int = 0) -> Tuple:
    ...
    # tf.padding  在周围，图像，
    #矩阵对齐
    #  -1 要做one-hot   label_all 不手动one-hot   故0
    #label填充的赋值0  会embedding_lookup 0的值，
    #但是value 填充-1 会全0  在乘以对应的矩阵，还是零，所以  终究不会影响
    #label 填充赋值-1，embedding_lookup会报错，从0开始的
    #InvalidArgumentError (see above for traceback): indices[1] = -1 is not in [0, 5)
