...
flags.DEFINE_string("logfile",'logs.dsin_ns.out',"log file to save")
...
#__file__
FLAGS.csvfile = FLAGS.csvfile + '_dsin_ns.csv' 
PATH = os.path.dirname(os.path.abspath(__file__))


#__file__
logging.basicConfig(filename=os.path.join(PATH,FLAGS.logfile),filemode='w', # w
...
USER_API_LEN = 10
USER_API_SUM = 1000
FILE_USER_API = os.path.join(PATH,"userapione.json")
    ...
    def __init__(self):
        ...
        with self.graph.as_default():
            ...
            with tf.variable_scope("encoder"):
                # Embedding
                self.enc = embedding(self.recent_behavior_ph,
...
#FLAGS.batch_size,USER_API_LEN
                ...
                self.enc += tf.cast(positional_encoding(N=tf.shape(self.recent_behavior_ph)[0],
                                                     T= USER_API_LEN,
    ...
    def build_inputs(self):
        ...
        with self.graph.as_default():
            ...
            with tf.name_scope('Inputs'):
                ...
                #self.recent_behavior_ph = tf.placeholder(tf.int32,[FLAGS.batch_size,USER_API_LEN],name= "recent_behavior_ph")
                self.recent_behavior_ph = tf.placeholder(tf.int32,[None,None],name= "recent_behavior_ph")


            with tf.name_scope("Embedding_layer"):
               #20*40*128
                self.ad_img_embeddings_var2  = tf.get_variable("adimg_embedding_var2", [AD_IMG_LABEL_DIM,AD_IMG_VALUE_DIM,EMBEDDING_DIM])
                #索引 idx  
                self.ad_img_embedded2 = tf.nn.embedding_lookup(self.ad_img_embeddings_var2, self.ad_label_ph)
                

                #n*7 ->  n*7*8  做one-hot  -1  00000   1   10000
                ...
                #n*7*8*128  就是对应相乘，
                self.adimg_embedded = tf.matmul(self.ad_value_ph_ohot ,self.ad_img_embedded2)

                
                self.adimg_eb = self.adimg_embedded     # none*n*1*128      

                self.adimg_eb = tf.squeeze(self.adimg_eb,[-2])  #n*n*128
            ...
            #with tf.name_scope("Embedding_layer"):
                #self.user_recent_behavior_var = tf.get_variable("user_recent_behavior_var", [USER_API_SUM, EMBEDDING_DIM])
                #self.recent_behavior_eb = tf.nn.embedding_lookup(self.user_recent_behavior_var, self.recent_behavior_ph)
    ...
    def train(self, sess, inps):
        loss, accuracy, _ = sess.run(
            ...
            feed_dict={
                ...
                self.recent_behavior_ph: inps[20]
            }
        )
        return loss, accuracy
    ...
    def calculate(self, sess, inps):
        probs, loss, accuracy= sess.run(
            ...
            feed_dict={
                ...
                self.recent_behavior_ph: inps[19]
    ...
    def build_tensor_info(self):
        ...
        #ad img
        add_ph = ["ad_label_ph","ad_value_ph","recent_behavior_ph"]
...
def process_rencent_behavior(r_behavior):
    ...
    for i in r_behavior:
        r_behavior_int.append(user_api_all[i]) 

    return  r_behavior_int
...
def prepare_data(feature: List, target: List, choose_len: int = 0) -> Tuple:
    ...
    rencent_behavior = [fea[21] for fea in feature]
    rencent_behavior= keras.preprocessing.sequence.pad_sequences(rencent_behavior,
    ...
    return user_id, ad_id, mobile, province, city, grade, math, english, \
           ...
           ad_his, ad_mask, np.array(lengths_xx), np.array(target), \
               label_all,value_all,rencent_behavior
...
def produce(filter_str, request):
    try:
        with HbaseDataIterUpdate("10.9.75.202", b'midas_offline_v1', filter_str, request,) as d:
            for i in d.get_data(batch_size=FLAGS.batch_size,model_num=0):
...
def get_max_model_index(cnt: int = 1):
    model_path = "tmp-model-3/modelds_ns/model/"
    if cnt != 1:
        model_path = "update-model-1/modelds_ns/model/"
...
def get_max_serving_index(cnt: int = 1):
    model_path = "tmp-model-3/modelds_ns/serving/"
    if cnt != 1:
        model_path = "update-model-1/modelds_ns/serving/"
...
if __name__ == "__main__":
    ...
    model_path = "update-model-1/modelds_ns/model/ckpt_"
    best_model_path = "update-model-1/modelds_ns/best-model/ckpt_"

    path1 ='update-model-1/modelds_ns'
                ...
                try:
                    ...
                    user_id, ad_id, mobile, province, city, grade, math, english, \
                        chinese, purchase, activity, freshness, hour, ad_his, mask, length, target, \
                            ad_label,ad_value,  rencent_behavior = prepare_data(feature,target)
            ...
            while True:
                ...
                try:
                    ...
                    prob, loss2,acc2  = model.test(sess, [user_id, ad_id, mobile, province, city, grade, math, english,\
                        chinese, purchase, activity, freshness, hour, ad_his, mask, length,\
                            target, 
                            ad_label,ad_value   ,rencent_behavior 
                                            ])
