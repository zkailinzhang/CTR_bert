...
#__file__
FLAGS.csvfile = FLAGS.csvfile + '_dsin_s.csv' 
PATH = os.path.dirname(os.path.abspath(__file__))

flags.DEFINE_string("train_cnt_file",os.path.join(PATH,"train_cnt_dsin_s.npy"),'file train cnt')
                ...
                for i in range(num_blocks):
                    with tf.variable_scope("num_blocks_{}".format(i)):
                        ...
                        self.enc = multihead_attention(queries = self.enc,
                                                       keys = self.enc,
                                                       num_units = hidden_units*3,
                                                       num_heads = num_heads,
                                                       dropout_rate = dropout_rate,
                                                       #is_training = is_training,
                                                       causality = False
                                                       )
                        self.enc = feedforward(self.enc,num_units = [4 * hidden_units,hidden_units*3])
...
def get_max_model_index(cnt: int = 1):
    model_path = "tmp-model-3/modeldsin_s/model/"
    if cnt != 1:
        model_path = "update-model-1/modeldsin_s/model/"
...
def get_max_serving_index(cnt: int = 1):
    model_path = "tmp-model-3/modeldsin_s/serving/"
    if cnt != 1:
        model_path = "update-model-1/modeldsin_s/serving/"
...
if __name__ == "__main__":
    ...
    model_path = "update-model-1/modeldsin_s/model/ckpt_"
    best_model_path = "update-model-1/modeldsin_s/best-model/ckpt_"

    path1 ='update-model-1/modeldsin_s'
