...
flags.DEFINE_string("logfile",'logs.dsin.s2.out',"log file to save")
...
#__file__
FLAGS.csvfile = FLAGS.csvfile + '_dsin_s2.csv' 
    ...
    def __init__(self):
        ...
        with self.graph.as_default():
            ...
            with tf.variable_scope("encoder"):
                ...
                self.enc_a += embedding(tf.tile(tf.expand_dims(tf.range(tf.shape(self.user_api_a_ph)[1]),0),[tf.shape(self.user_api_a_ph)[0],1]),
                ...
                self.enc_b = embedding(self.user_api_b_ph,
                                     ...
                                     scale=True,
                                     scope="enc_embed_b")
                self.enc_b += embedding(tf.tile(tf.expand_dims(tf.range(tf.shape(self.user_api_b_ph)[1]),0),[tf.shape(self.user_api_b_ph)[0],1]),
                ...
                self.enc_c = embedding(self.user_api_c_ph,
                                     ...
                                     scale=True,
                                     scope="enc_embed_c")
                self.enc_c += embedding(tf.tile(tf.expand_dims(tf.range(tf.shape(self.user_api_c_ph)[1]),0),[tf.shape(self.user_api_c_ph)[0],1]),
...
def get_max_model_index(cnt: int = 1):
    model_path = "tmp-model-3/modeldsin_s2/model/"
    if cnt != 1:
        model_path = "update-model-1/modeldsin_s2/model/"
...
def get_max_serving_index(cnt: int = 1):
    model_path = "tmp-model-3/modeldsin_s2/serving/"
    if cnt != 1:
        model_path = "update-model-1/modeldsin_s2/serving/"
...
if __name__ == "__main__":
    ...
    model_path = "update-model-1/modeldsin_s2/model/ckpt_"
    best_model_path = "update-model-1/modeldsin_s2/best-model/ckpt_"

    path1 ='update-model-1/modeldsin_s2'
