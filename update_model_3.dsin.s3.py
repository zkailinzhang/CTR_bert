...
flags.DEFINE_string("logfile",'logs.dsin.s3.out',"log file to save")
...
#__file__
FLAGS.csvfile = FLAGS.csvfile + '_dsin_s3.csv' 
...
#文件
FILE_USER_API = os.path.join(PATH,"userapi.json")
        ...
        with self.graph.as_default():
            ...
            with tf.variable_scope("encoder"):
                ...
                self.enc_b = embedding(self.user_api_b_ph,
                                     ...
                                     scale=True,
                                     scope="enc_embed_b")
                self.enc_b += tf.cast(positional_encoding(N=tf.shape(self.user_api_b_ph)[0],
                                                      T= USER_API_LEN,
                                                    num_units = hidden_units,
                                                    zero_pad = False,
                                                    scale = False,
                ...
                self.enc_c = embedding(self.user_api_c_ph,
                                     ...
                                     scale=True,
                                     scope="enc_embed_c")
                self.enc_c += tf.cast(positional_encoding(N=tf.shape(self.user_api_c_ph)[0],
                                                      T= USER_API_LEN,
                                                    num_units = hidden_units,
                                                    zero_pad = False,
                                                    scale = False,
                ...
                for i in range(num_blocks):
                    with tf.variable_scope("num_blocks_a_{}".format(i)):
                        ### MultiHead Attention  
                        #[128, 30, 512] 不变
                        self.enc_a = multihead_attention(queries = self.enc_a,
                                                       keys = self.enc_a,
                                                       num_units = hidden_units,
                                                       num_heads = num_heads,
                        ...
                        self.enc_a = feedforward(self.enc_a,num_units = [4 * hidden_units,hidden_units])


                for i in range(num_blocks):
                    with tf.variable_scope("num_blocks_b_{}".format(i)):
                        ### MultiHead Attention  
                        #[128, 30, 512] 不变
                        self.enc_b = multihead_attention(queries = self.enc_b,
                                                       keys = self.enc_b,
                                                       num_units = hidden_units,
                                                       num_heads = num_heads,
                                                       dropout_rate = dropout_rate,
                        ...
                        self.enc_b = feedforward(self.enc_b,num_units = [4 * hidden_units,hidden_units])

                for i in range(num_blocks):
                    with tf.variable_scope("num_blocks_c_{}".format(i)):
                        ### MultiHead Attention  
                        #[128, 30, 512] 不变
                        self.enc_c = multihead_attention(queries = self.enc_c,
                                                       keys = self.enc_c,
                                                       num_units = hidden_units,
                                                       num_heads = num_heads,
                                                       dropout_rate = dropout_rate,
                        ...
                        self.enc_c = feedforward(self.enc_c,num_units = [4 * hidden_units,hidden_units])

            # Final linear projection
            #self.logits = tf.layers.dense(self.dec,USER_API_LEN*3))
            #print(self.enc.get_shape().as_list())
            self.user_api_a_eb_sum = tf.reduce_sum(self.enc_a,-2)
            self.user_api_b_eb_sum = tf.reduce_sum(self.enc_b,-2)
            self.user_api_c_eb_sum = tf.reduce_sum(self.enc_c,-2)
            #self.enc = tf.concat([self.enc_a,self.enc_b,self.enc_c],-2)
            ...
            inp = tf.concat(
                [self.item_eb, self.item_his_eb_sum, self.item_eb * self.item_his_eb_sum,
                 ...
                 self.ad_img_eb_sum,
                 self.user_api_a_eb_sum,
                 self.user_api_b_eb_sum,
                 self.user_api_c_eb_sum
                 ], -1)
...
def get_max_model_index(cnt: int = 1):
    model_path = "tmp-model-3/modeldsin_s3/model/"
    if cnt != 1:
        model_path = "update-model-1/modeldsin_s3/model/"
...
def get_max_serving_index(cnt: int = 1):
    model_path = "tmp-model-3/modeldsin_s3/serving/"
    if cnt != 1:
        model_path = "update-model-1/modeldsin_s3/serving/"
...
if __name__ == "__main__":
    ...
    model_path = "update-model-1/modeldsin_s3/model/ckpt_"
    best_model_path = "update-model-1/modeldsin_s3/best-model/ckpt_"

    path1 ='update-model-1/modeldsin_s3'
