...
flags.DEFINE_string("logfile",'logs.base_dien.lr0.8.out',"log file to save")
...
#__file__
FLAGS.csvfile = FLAGS.csvfile+ str(FLAGS.decay_rate) + '_base_dien.csv' 
...
USER_API_SUM_B = 200
USER_API_SUM_C = 1000
FILE_USER_API = os.path.join(PATH,"apisplit.json")
        ...
        with self.graph.as_default():
            ...
            with tf.name_scope("Self_Attention_layer"):
               #out =  transformer(self.user_api_all_eb)
                print(self.user_api_all_eb.get_shape().as_list())
                print("xxx")
                a = input()

                Self_Attention = Transformer(att_embedding_size=1, head_num=8, dropout_rate=0, use_layer_norm=False,
                                 use_positional_encoding=True, seed=1024, supports_masking=True,
                                 blinding=True)

                self.selfatte_out = Self_Attention([self.user_api_all_eb,self.user_api_all_eb])
                
                
                print(self.selfatte_out.get_shape().as_list())
                print("xx")
                a = input()
...
def process_rbehavior_split(r_behavior):
    ...
    try:
        if os.path.exists(FILE_USER_API):
            ...
            #第一个批次里的第一条数据  不存在创建
            user_api_a ,user_api_b ,user_api_c ={},{},{}
    ...
    for api in r_behavior:
        ...
        if api_sp == ['']:
            ...
            rbehavior_int_a.append(user_api_a[''])
            rbehavior_int_b.append(user_api_b[''])
            rbehavior_int_c.append(user_api_c[''])
        else:
            rbehavior_int_a.append(user_api_a[api_sp[1]])  
            rbehavior_int_b.append(user_api_b[api_sp[2]]) 
            rbehavior_int_c.append(user_api_c[api_sp[3]]) 
...
def produce(filter_str, request):
    try:
        ...
        logging.debug("*** HbaseDataIterUpdate except ***")
        pass      
    finally:
...
def get_max_model_index(cnt: int = 1):
    model_path = "tmp-model-3/modeldien0.8/model/"
    if cnt != 1:
        model_path = "update-model-1/modeldien0.8/model/"
...
def get_max_serving_index(cnt: int = 1):
    model_path = "tmp-model-3/modeldien0.8/serving/"
    if cnt != 1:
        model_path = "update-model-1/modeldien0.8/serving/"
...
if __name__ == "__main__":
    ...
    model_path = "update-model-1/modeldien0.8/model/ckpt_"
    best_model_path = "update-model-1/modeldien0.8/best-model/ckpt_"

    path1 ='update-model-1/modeldien0.8'
    ...
    with tf.Session(graph=model.graph) as sess:
        ...
        curentday = start_day
        if iterday.strftime("%Y-%m-%d") != start_day:
...
##############################         
            ...
            while True:
                ...
                if iiter % save_iter == 0:
                    ...
                    model.save_serving_model(sess, os.path.join(PATH, "update-model-1", "modeldien0.8","serving"), version=version)
