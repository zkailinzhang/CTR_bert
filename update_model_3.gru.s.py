...
flags.DEFINE_string("logfile",'logs.ds_g.out',"log file to save")
...
#__file__
FLAGS.csvfile = FLAGS.csvfile + '_ds_g.csv' 
        ...
        with self.graph.as_default():
            ...
            with tf.name_scope("Self_Attention_layer"):
               #out =  transformer(self.user_api_all_eb)
                ...
                #cell = tf.nn.rnn_cell.GRUCell(HIDDEN_DIM*2)  error  需要另外一种分时写
                    def get_gru():
                        cell = tf.nn.rnn_cell.GRUCell(HIDDEN_DIM*3)
                    #cell = tf.contrib.rnn.GRUCell(HIDDEN_DIM)
                        cell = tf.nn.rnn_cell.DropoutWrapper(cell, output_keep_prob=KEEP_PROB)
                    #cell = tf.contrib.rnn.DropoutWrapper(cell, output_keep_prob=KEEP_PROB)
                        return cell
                    cells = [get_gru() for _ in range(num_layers)]
                    Cell = tf.nn.rnn_cell.MultiRNNCell(cells, state_is_tuple=True)
                    #Cell =  tf.contrib.rnn.MultiRNNCell(cells,state_is_tuple=True)

                with tf.name_scope('gru'):

                    rnnoutput, _ = tf.nn.dynamic_rnn(cell=Cell, inputs=self.user_api_all_eb, dtype=tf.float32)
                    #rnnoutput,_ = tf.contrib.rnn.dynamic_rnn()
                    rnnoutput = tf.reduce_sum(rnnoutput, axis=-2)
                self.user_api_eb_sum = rnnoutput  
...
def get_max_model_index(cnt: int = 1):
    model_path = "tmp-model-3/modelds_g/model/"
    if cnt != 1:
        model_path = "update-model-1/modelds_g/model/"
...
def get_max_serving_index(cnt: int = 1):
    model_path = "tmp-model-3/modelds_g/serving/"
    if cnt != 1:
        model_path = "update-model-1/modelds_g/serving/"
...
if __name__ == "__main__":
    ...
    model_path = "update-model-1/modelds_g/model/ckpt_"
    best_model_path = "update-model-1/modelds_g/best-model/ckpt_"

    path1 ='update-model-1/modelds_g'
