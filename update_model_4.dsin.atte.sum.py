...
flags.DEFINE_string("logfile",'logs.dsin_4_1.out',"log file to save")
...
#__file__
FLAGS.csvfile = FLAGS.csvfile + '_dsin_4_1.csv' 
PATH = os.path.dirname(os.path.abspath(__file__))

flags.DEFINE_string("train_cnt_file",os.path.join(PATH,"train_cnt_dsin_4_1.npy"),'file train cnt')
    ...
    def __init__(self):
        ...
        with self.graph.as_default():
            ...
            with tf.variable_scope("encoder"):
                ...
                #静态建图的话，这个变量即使中间改变，网络也不会变了吧
                print("gra AD_hist_len:",AD_HIST_LEN)
                if sinusoid:
                    self.enc_ad += tf.cast(positional_encoding(  #N=FLAGS.batch_size,
                                                     N = tf.shape(self.mid_his_ph)[0],
                                                     #T = tf.shape(self.mid_his_ph)[1],#AD_SUM,
                                                    # T= self.mid_his_ph.get_shape().as_list()[1],
                                                    T= AD_HIST_LEN,
            ...
            #tf.nn.sigmoid  80 40   (64,16),att_activation="dice",  att_hidden_units=(80,40),att_activation=tf.nn.sigmoid,
            self.mask_ph_2 = tf.ones(shape=[tf.shape(self.enc)[0],tf.shape(self.enc)[1]],dtype=tf.bool)
            self.final_state2 = AttentionSequencePoolingLayer(
            weight_normalization=False, supports_masking=True)(
            inputs = [self.item_eb_ex , self.enc],mask=self.mask_ph_2)
            
            self.final_state3 = tf.reduce_sum(self.final_state2,-2)
...
def prepare_data(feature: List, target: List, choose_len: int = 0) -> Tuple:
    ...
    #AD_hist_len = max_len
    print("pre AD_hist_len:",AD_HIST_LEN)
    ...
    ad_mask = keras.preprocessing.sequence.pad_sequences(ad_mask,
    maxlen=AD_HIST_LEN,padding='post',value=0)
    
    ad_his = keras.preprocessing.sequence.pad_sequences(ad_his,
    maxlen=AD_HIST_LEN,padding='post',value=0)
...
def get_max_model_index(cnt: int = 1):
    model_path = "tmp-model-3/modeldsin_4_1/model/"
    if cnt != 1:
        model_path = "update-model-1/modeldsin_4_1/model/"
...
def get_max_serving_index(cnt: int = 1):
    model_path = "tmp-model-3/modeldsin_4_1/serving/"
    if cnt != 1:
        model_path = "update-model-1/modeldsin_4_1/serving/"
...
if __name__ == "__main__":
    ...
    model_path = "update-model-1/modeldsin_4_1/model/ckpt_"
    best_model_path = "update-model-1/modeldsin_4_1/best-model/ckpt_"

    path1 ='update-model-1/modeldsin_4_1'
